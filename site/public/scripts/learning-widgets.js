(function () {
  ...
  const REVIEW_BANKS = {
    1: [
      { prompt: "What makes a study question easy to answer and check?", options: ["It names one thing, what you want to know about it, and a way to check the answer", "It sounds technical", "It has the longest possible wording", "It can be answered without looking anything up"], answer: 0, explanation: "A small question with a named check tells you when the answer is complete and whether it is right." },
      ...
      { prompt: "An f32's 8-bit exponent field holds 126. What does the value get multiplied by?", options: ["One half, because 126 − 127 = −1 and 2⁻¹ is ½", "2¹²⁶, because the field holds 126", "126 × 2", "Nothing, because 126 marks the number as zero"], answer: 0, explanation: "The stored exponent is the real exponent plus the bias of 127, so a stored 126 means 2⁻¹. The bias lets an eight-bit field hold negative exponents without a minus sign." },
      ...
      { prompt: "What should you do when an AI supplies an offset absent from the provided source?", options: ["Treat it as unverified and check an authoritative source or controlled experiment", "Use it because offsets cannot be invented", "Patch every matching process", "Delete the version check from your tool"], answer: 0, explanation: "An AI can summarize material you give it, but an offset it cannot trace to a source or a test is a guess." },
      ...
      { prompt: "Why change one thing at a time during an experiment?", options: ["It makes the game run faster", "It connects one cause to one observed effect", "It makes every scan faster", "It makes every address permanent"], answer: 1, explanation: "With one change, any difference in memory has one candidate cause. Several changes at once make the result ambiguous." },
      ...
      { prompt: "Why does an f32 not store the 1 in front of the binary point?", options: ["In binary scientific notation that digit is always 1 for normal numbers, so storing it would waste a bit", "The CPU cannot store the digit 1", "It is kept in the sign bit instead", "Only negative numbers have it"], answer: 0, explanation: "Binary has only one non-zero digit, so after sliding the point a 1 always sits in front. Leaving it out gives an f32 24 bits of precision from 23 stored fraction bits." },
      { prompt: "A 64-bit game holds a *const u8 and a *const Player. What does the pointee type change?", options: ["How many bytes are read at the destination and how far add(1) steps, not the pointer's own 8-byte size", "The pointer's own size: 1 byte for u8, larger for Player", "Nothing; pointee types are only comments", "Which process the address belongs to"], answer: 0, explanation: "Every pointer in a 64-bit process is 8 bytes. The pointee type tells the compiled code what to read at the address and how far pointer arithmetic steps." },
      ...
      { prompt: "What separates a game engine from a library?", options: ["Who calls whom: your code calls a library, while an engine runs the loop and calls the game's code every frame", "An engine is always larger", "A library can only draw graphics", "An engine is written in C++ and a library in C"], answer: 0, explanation: "The difference is control. A library waits to be asked. An engine owns the game loop and calls the game's update and draw code at moments it chooses." },
      { prompt: "Why do an engine's console variables and debug commands make good anchors for an experiment?", options: ["Toggling one gives a clean cause, so whatever changes in memory at the same moment is connected to that feature", "They disable anti-cheat", "They are the only way to read memory", "They make every address permanent"], answer: 0, explanation: "Switching one feature on and off gives one clear cause, so memory that changes at that moment is likely tied to that feature. Engines often ship these tools because their own developers needed them." },
      { prompt: "Why does a game's main loop check for window messages with PeekMessage rather than GetMessage?", options: ["GetMessage waits until a message arrives, but a game must keep updating and drawing when nothing happens", "PeekMessage reads memory faster", "GetMessage only works in 64-bit programs", "PeekMessage delivers messages in a different order"], answer: 0, explanation: "A text editor can sleep until the next key press. A game's world keeps moving, so it takes whatever messages are waiting without stopping, then runs its frame." },
      { prompt: "What happens while one event handler takes a long time to finish?", options: ["Every event queued behind it waits, so input and drawing stall", "Windows runs the other handlers in parallel", "The slow handler is skipped", "Nothing; handlers never affect each other"], answer: 0, explanation: "An event loop handles one event at a time. A slow handler holds up the whole queue, which is why Windows marks a window Not Responding when its messages go unread for about five seconds." },
      { prompt: "Why does an external tool poll a game's memory instead of waiting for an event?", options: ["Nothing sends the tool an event when a game value changes, so it has to look on its own schedule", "Polling always uses less CPU than events", "Windows forbids events between processes", "Polling pauses the game while it reads"], answer: 0, explanation: "An event needs a sender. The game notifies no outside reader, so the tool checks repeatedly, and it can turn differences between snapshots into events of its own." },
      { prompt: "Why can a debugger set a breakpoint by writing a single byte into a running program?", options: ["Instructions sit in memory as ordinary bytes, so overwriting one changes what the CPU executes when it arrives there", "Debuggers switch the CPU into a mode that ignores memory", "Windows keeps a hidden copy of every instruction that the debugger edits", "The byte changes only what the debugger displays"], answer: 0, explanation: "A stored-program computer keeps code and data in the same memory. Writing 0xCC over an instruction's first byte turns it into int3, and the CPU stops there for the debugger." },
      { prompt: "What does it mean that a question about programs is undecidable?", options: ["No single method answers it correctly for every possible program, although many specific programs can still be analyzed", "Nobody has studied the question yet", "It can be answered only for programs written in C", "Every possible answer to it is false"], answer: 0, explanation: "The limit is on one method that works for all programs. Real tools approximate, and each approximation has a known way to be wrong, which is why the book pairs reading code with running experiments." },
      { prompt: "Why is a mod that ships a script treated differently from a mod that ships a texture?", options: ["A script in a Turing-complete language is a program that can compute anything, limited only by what its host exposes", "Scripts are always larger than textures", "Textures cannot contain bytes", "Scripts are read only once, at install time"], answer: 0, explanation: "A texture is data the game reads in one fixed way. A script is a program, so what it may touch depends entirely on the host's rules, which is why Chapter 12 cares about what a Lua host hands to its scripts." },
    ],
    2: [
      ...
      { prompt: "A write breakpoint hits once while you buy a unit. What should you check before calling that instruction the gold update?", options: ["That it hits again on every purchase, and not when you do something else", "That the address looks familiar", "That the register holds a large number", "Nothing, because one hit is enough"], answer: 0, explanation: "Code that runs for many reasons can hit once by coincidence. An instruction that runs on every purchase, and only then, is tied to the purchase." },
    ...
    4: [
      ...
      { prompt: "A guard faces (1, 0). The unit direction to a player is (0.8, 0.6). With a view-cone threshold of 0.5, is the player inside the cone?", options: ["Yes, because 0.8 × 1 + 0.6 × 0 = 0.8, which is at least 0.5", "No, because 0.6 is less than 0.8", "Yes, because both numbers are positive", "No, because 0.8 + 0.6 = 1.4 is more than 1"], answer: 0, explanation: "For unit vectors the dot product is the cosine of the angle between them. 0.8 is at least cos 60° = 0.5, so the player is less than 60° from the facing direction." },
      { prompt: "Why does an NPC make its decision once per simulation tick rather than once per drawn frame?", options: ["Tying decisions to simulated time keeps reaction times and behavior the same at any frame rate", "Drawing is not allowed while AI code runs", "Frames are always drawn less often than ticks", "The dot product can only be computed once per tick"], answer: 0, explanation: "A guard that reacts after 3 ticks of 100 ms reacts after the same time on a fast or slow computer. Deciding per frame would make a machine drawing 144 frames per second react faster than one drawing 30." },
      { prompt: "A guard needs to see the player on 3 consecutive ticks, and the AI runs 10 ticks per second. How long after the first tick that sees the player does the chase begin?", options: ["200 milliseconds: the count reaches 3 two ticks later, and each tick is 100 ms", "300 milliseconds: three ticks of 100 ms", "30 milliseconds", "On the same tick"], answer: 0, explanation: "Ticks are 1000 ÷ 10 = 100 ms apart. The first sighting sets the count to 1, and it reaches 3 two ticks later, 2 × 100 = 200 ms. The player may also have been visible for up to 100 ms before that first tick sensed them." },
      { prompt: "Why does the lab's `can_see` check distance and the view cone before line of sight?", options: ["They cost a few arithmetic operations, while line of sight walks up to 100 samples and tests every wall", "Line of sight only works for close targets", "The order of the checks does not affect the cost", "The view cone needs the line-of-sight result first"], answer: 0, explanation: "The function returns as soon as one check fails. The ray walk takes distance ÷ 0.1 samples, up to 10 ÷ 0.1 = 100, each tested against every wall, so it runs only when the cheap checks have passed." },
      { prompt: "With health 0.3, an enemy in view, no cover, and one potion, the scores are Attack 0.4 + 0.6 × 0.3 = 0.58, TakeCover 0.9 − 0.6 × 0.3 = 0.72, and Heal 1.0 − 0.3 = 0.70. What does utility scoring choose?", options: ["TakeCover, because it has the highest score", "Heal, because health is low", "Attack, because it is listed first", "Nothing, because two scores are close"], answer: 0, explanation: "Utility scoring runs the highest-scoring option. 0.72 beats 0.70 and 0.58. Healing only wins below 25% health, where 1.0 − h overtakes 0.9 − 0.6 × h." },
      { prompt: "Why can editing your own client's memory not change an NPC's decisions in an online game?", options: ["The server runs the NPC's state machine; the client holds only copies for drawing, which the next server update overwrites", "Every client encrypts NPC memory", "Online NPCs have no state", "The client runs the AI twice and keeps the second result"], answer: 0, explanation: "The server's copy of the game state is authoritative. The NPC's state, timers, and decision code live in the server's memory; changing a client-side copy affects at most what your own screen draws." },
    ],
    5: [
    ...
    7: [
      ...
      { prompt: "What does a second value scan keep?", options: ["Old candidates that now hold the new value", "Every address in the process", "Only executable pages", "Only the first result"], answer: 0, explanation: "Each scan filters the existing candidate set instead of starting over." },
    ...
    8: [
      ...
      { prompt: "What does a controlled diff between two save files reveal?", options: ["Which byte ranges or fields changed with one known game action", "The live call stack", "The GPU projection matrix", "The process DACL"], answer: 0, explanation: "When only one in-game value changed, the differing bytes show where that value, and anything computed from it such as a checksum, is stored." },
      ...
      { prompt: "A hex editor shows 16 bytes per row. Why does the second row's offset column read 00000010?", options: ["The row starts at byte 16, and 16 is 0x10 in hex", "Each row holds ten bytes", "The editor numbers rows, not bytes", "Every file header is exactly 16 bytes long"], answer: 0, explanation: "The offset column gives the position of each row's first byte, in hex. With 16 bytes per row, the second row starts at byte 16, which is 1 × 16 + 0 = 0x10." },
      { prompt: "A save stores gold as the little-endian bytes FA 00 00 00. What value is that?", options: ["250", "4,194,304,000", "64,000", "15"], answer: 0, explanation: "Little endian puts the byte worth the least first. 0xFA is 15 × 16 + 10 = 250, and the other three bytes are zero. Read in the opposite order, the same bytes would be 0xFA000000 = 4,194,304,000, an obviously wrong gold count." },
      { prompt: "What does a magic number at the start of a file let a loader do?", options: ["Recognize the file's format before trusting any other field", "Decrypt the rest of the file", "Find where the file will sit in memory", "Skip checksum validation"], answer: 0, explanation: "A fixed byte pattern such as GSAV or the PNG signature identifies the format. A file with the wrong magic is refused before any of its bytes are read as fields." },
      { prompt: "A save stores a name as a one-byte length followed by the text, with a checksum right after the name. The name changes from Ada to Grace. What happens to the checksum?", options: ["It moves from offset 15 to offset 17, and its value must be recomputed", "It stays at offset 15 with the same value", "It disappears because the file grew", "Only the length byte changes; nothing after it moves"], answer: 0, explanation: "The checksum starts at 12 + name length, so it moves from 12 + 3 = 15 to 12 + 5 = 17. The length byte and the name bytes both changed, so the sum they feed into changes as well." },
      { prompt: "Two saves differ in one in-game value, yet a hex compare also shows a byte near the end of the file changing. What is the most likely explanation?", options: ["The file carries a checksum computed over the other bytes", "The hex editor misread the file", "The save is encrypted", "The game stores every value twice"], answer: 0, explanation: "A checksum depends on every byte it covers, so it changes whenever any covered value changes. Raising gold from 250 to 1,250 in the GSAV example changes offsets 6 and 7 and the checksum byte at offset 15." },
      { prompt: "Why can a hex editor not change the gold in a save protected with authenticated encryption?", options: ["The gold is not stored as plain bytes, and any change to the ciphertext makes the loader refuse the file", "Hex editors cannot open encrypted files", "Encryption always stores numbers big endian", "The checksum is always stored at offset zero"], answer: 0, explanation: "Ciphertext has no field that can be located and overwritten. Changing it alters the decrypted result in ways nobody can choose, and the authentication check rejects any modified file. Without the key there is nothing to edit." },
    ],
    9: [
      ...
      { prompt: "Why is a small crash dump often preferable to dumping all memory?", options: ["It can capture relevant threads, modules, and contexts with less sensitive or irrelevant data", "It keeps the process running forever", "It contains source code", "It disables ASLR"], answer: 0, explanation: "Selectivity reduces size and data exposure while keeping the threads, modules, and stacks needed to diagnose a scoped crash." },
      ...
      { prompt: "What should happen when a page-table present bit is clear?", options: ["Return a level-specific error", "Read address zero", "Guess the next table", "Mask away the error bit"], answer: 0, explanation: "A clear present bit means this address space has no mapping there in the capture. Guessing would turn malformed or wrong-address-space data into misleading output." },
      { prompt: "Why does a capture file travel with its SHA-256 digest?", options: ["It lets later analysis verify that the capture's bytes have not changed", "It decrypts the capture", "It reveals every virtual address", "It grants hardware access"], answer: 0, explanation: "A digest detects accidental or unexplained changes to the image, so every translation can be tied to the exact bytes it was made from." },
    ...
    10: [
      ...
      { prompt: "Why does the book identify a game's engine and runtime before scanning for offsets?", options: ["The runtime decides whether \"a field at a fixed offset from an object base\" is even a durable fact", "Newer engines are always harder to study", "It is only needed for multiplayer games", "Offset scanning requires the engine's source code"], answer: 0, explanation: "For native C++ builds the assumption holds. For managed or JIT runtimes the number you want may never have been stored at a fixed offset." },
      ...
      { prompt: "Why check a game's folder against the list of modules the process actually loaded?", options: ["A folder can be stale, and a launcher can start something other than the executable you inspected", "Module lists are faster to read", "Folders never contain useful files", "Loaded modules reveal the source code"], answer: 0, explanation: "The folder shows what is installed. The module list shows what the running process mapped, which is the code that actually executes." },
    ...
    13: [
      { prompt: "x64dbg shows `cs` = `0033` for a 64-bit program. Which part of that value is the privilege level, and what level is it?", options: ["The lowest two bits, `11`, so ring 3", "The whole value, 51, which means user mode", "The top hex digit, 0, so ring 0", "The second hex digit, 3, which counts the program's threads"], answer: 0, explanation: "`0x33` is `0011 0011` in binary. Only its lowest two bits hold the current privilege level, and `11` is 2 + 1 = 3: ring 3, user mode. Two bits give 2 × 2 = 4 patterns, which is why x86 has four rings. The kernel's code segment, `0x10` = `0001 0000`, ends in `00`, ring 0." },
      { prompt: "What makes the `syscall` instruction a safe way for user-mode code to enter the kernel?", options: ["It checks the caller's password before switching modes", "It copies the whole program into kernel memory first", "It jumps to an address the kernel stored at boot in a setting only ring 0 can write, so a program chooses when to enter but never where", "It works only for programs signed by Microsoft"], answer: 2, explanation: "`syscall` saves the return address in `rcx` and the flags in `r11`, switches to ring 0, and jumps to the kernel's chosen entry point. Every system call from every program arrives at the same code, which checks the request before doing anything with it, so no program can land past the checks." },
      { prompt: "Why does Windows stop the whole machine with a bug check when a driver unexpectedly follows a bad pointer, instead of ending just that driver?", options: ["Drivers are ordinary processes, and ending one always restarts Windows", "All kernel code and drivers share one address space and one set of data structures, so the kernel cannot know what the fault left half updated, and continuing could corrupt data on disk", "The processor cannot raise page faults in ring 0", "A bug check is how Windows reports that a driver is unsigned"], answer: 1, explanation: "A user-mode fault is caught by the kernel, which ends one process. A fault in kernel code has no layer beneath it, and it may have struck in the middle of updating the scheduler's lists or the memory manager's records with a lock held. Windows halts deliberately, shows a stop code, and writes a crash dump rather than risk writing corrupted data." },
      { prompt: "What is an I/O request packet (IRP)?", options: ["A network packet that a device sends to the processor", "The machine code of a driver", "A user-mode buffer that the driver writes into directly", "The kernel's work order for one device request: it names the operation, carries the parameters and buffers, travels down the driver stack, and comes back with a status"], answer: 3, explanation: "The I/O manager builds an IRP for each `ReadFile`, `WriteFile`, or `DeviceIoControl` call and passes it down the driver stack. A driver completes it at once or keeps it pending until its device interrupts, then completes it with a status and a byte count, and the I/O manager returns the result to the caller." },
      { prompt: "The toy keypad's STATUS register reads `0x0000_0201`. Bit 0 is data ready, bit 1 is overflow, and bits 8 to 15 count waiting events. What does the device report?", options: ["Data ready, no overflow, and 2 events waiting", "513 events waiting", "Overflow, and 1 event waiting", "The device is disabled"], answer: 0, explanation: "The low byte, `0x01`, has only bit 0 set: data ready, no overflow. The next byte, `0x02`, is the count in bits 8 to 15: 2 events. As one number `0x0201` = 2 × 256 + 1 = 513, and dividing by 256 gives 2 remainder 1, the same two fields." },
      { prompt: "Why does Windows refuse to load some drivers that carry a valid signature?", options: ["Driver signatures expire every day", "Windows loads only drivers installed through Windows Update", "A signature proves who published a driver and that it has not changed, not that its code is safe, so signed drivers with known exploitable bugs are placed on the vulnerable driver blocklist", "Only drivers written in Rust are allowed to load"], answer: 2, explanation: "Attackers look for old, legitimately signed drivers whose control codes read or write any memory, and load one to borrow its bug. The vulnerable driver blocklist lists such drivers. It is on by default since the Windows 11 2022 update and enforced whenever memory integrity is on." },
      { prompt: "What happens during a VM exit?", options: ["The guest operating system shuts down cleanly", "The processor saves the guest's state, loads the host's, and runs the hypervisor's exit handler with a reason code; the hypervisor handles the cause and enters the guest again", "A program inside the guest exits and returns to the guest kernel", "The hypervisor copies the guest's disk into a snapshot"], answer: 1, explanation: "Exits happen for actions the hypervisor listed in its control structure, such as `cpuid`, port I/O, or an access to a guest physical page with no stage-2 mapping. The hypervisor emulates the effect, moves the guest past the instruction, and resumes it, so from inside the guest the instruction simply took effect. Each round trip costs hundreds to thousands of cycles, which is why hypervisors keep exits rare." },
      { prompt: "With 4 KiB pages, guest virtual address `0x0040_3A2C` translates through guest frame `0x1F7` to host frame `0x52E3`. Which part of the address stays the same through both stages, and why?", options: ["The first three hex digits, because they name the process", "Every digit changes at each stage", "Only the lowest bit, which marks the page as present", "The last three hex digits, `0xA2C`: 2¹² = 4,096, so the low 12 bits are the offset inside the page, and each stage only replaces the page or frame number above them"], answer: 3, explanation: "12 bits are exactly three hex digits, so `0xA2C` is the offset, 10 × 256 + 2 × 16 + 12 = 2,604. Stage 1 gives the guest physical address `0x1F7` × `0x1000` + `0xA2C` = `0x001F_7A2C`, and stage 2 gives the host physical address `0x52E3` × `0x1000` + `0xA2C` = `0x052E_3A2C`." },
      { prompt: "What was JTAG originally designed to do?", options: ["Test the solder joints on assembled circuit boards, through a boundary-scan cell on each chip pin", "Encrypt the data that chips send to each other", "Let games read controllers faster", "Replace the operating system's debugger API"], answer: 0, explanation: "Chips had moved their pins where a bed-of-nails fixture could not reach them, so JTAG built the probes into the chips. Each pin gets a cell that can capture or drive its level, and the cells form a shift register that a tester reads and writes through one serial chain. On-chip debugging came later, through the same port." },
      { prompt: "A debugger has lost track of which state the TAP controller is in. What always returns the controller to Test-Logic-Reset?", options: ["Five clocks with TMS held at 1", "Five clocks with TDI held at 1", "One clock with TMS at 0", "Sixteen clocks with TMS held at 0"], answer: 0, explanation: "With TMS at 1, every state moves toward Test-Logic-Reset, and the longest path, for example Shift-DR, Exit1-DR, Update-DR, Select-DR-Scan, Select-IR-Scan, Test-Logic-Reset, takes five clocks. TDI never steers the controller, and TMS at 0 keeps it waiting in states such as Shift-DR." },
      { prompt: "How does SWD differ from JTAG?", options: ["It uses two wires, a clock and a shared data wire, and sends request packets, but reaches the same debug port", "It cannot halt the processor, only read memory", "It works only after the operating system has started", "It replaces the debug port with a password prompt"], answer: 0, explanation: "Serial Wire Debug trades JTAG's four signals and state machine for SWCLK and SWDIO and an 8-bit request such as 0xA5, which reads the debug port's ID register. Behind either protocol sits the same debug port, so halting, stepping, and memory access work the same way." },
      { prompt: "What does unified memory give a console that a typical gaming PC lacks?", options: ["The CPU and GPU read and write the same physical memory, so data is not copied between separate pools", "Memory that nobody can read, including the game", "A separate video RAM chip for every CPU core", "Memory that keeps its contents with the power off"], answer: 0, explanation: "A gaming PC keeps system RAM for the CPU and video RAM on the graphics card, and copies textures and buffers across the PCI Express bus. A console's CPU writes a buffer and its GPU reads the very same bytes, from one fast pool built from graphics memory: 448 GB/s on the PlayStation 5." },
      { prompt: "In a secure boot chain of trust, what does each stage do before it runs the next?", options: ["Checks the next stage's signature with a key it already trusts, and refuses to run it if the check fails", "Decrypts the next stage with the owner's password", "Asks the online service for permission", "Copies the next stage into the boot ROM"], answer: 0, explanation: "The boot ROM, which cannot change, checks the bootloader's key against a hash in fuses, then its signature and security version. Each later stage checks the one after it in the same way, so every piece of running code is trusted only because an earlier, already-trusted stage verified it." },
      { prompt: "A loop of 5 guest instructions runs 1,000 times. How many instruction decodes does an interpreter perform, compared with a dynamic recompiler that translates the loop once?", options: ["5,000 compared with 5", "5 compared with 5,000", "1,000 compared with 1", "5,000 compared with 5,000"], answer: 0, explanation: "An interpreter fetches and decodes every instruction every time it runs: 5 × 1,000 = 5,000 decodes. A recompiler decodes the 5 instructions once, stores host machine code for the block, and runs that stored code on each of the 1,000 passes." },
      { prompt: "An emulator's frame ends after step 8 of Lesson 14.11's trace, with A = 20 while gold in RAM still holds 15. Why must a save state include the registers, not only RAM?", options: ["The new total exists only in register A, so restoring RAM alone would lose it and resume from a state the machine was never in", "Registers are larger than RAM", "RAM cannot be copied while the emulator is paused", "The display register stores the program"], answer: 0, explanation: "A save state is the whole machine: registers, RAM, I/O registers, and timing position. At that instant, the program has computed 20 in A but not yet stored it. Restoring RAM without A, X, Z, and PC would give a machine that never existed, while loading the complete state continues exactly where the run stopped." },
      { prompt: "Which of these is legitimate to run in an emulator?", options: ["A homebrew game that its author released for anyone to play", "A console BIOS file downloaded from a file-sharing site", "A copy of a commercial game from someone else's download folder", "Firmware bundled with keys someone else extracted from a console"], answer: 0, explanation: "Emulators themselves are legal in many countries, but BIOS files, system firmware, and games are copyrighted, and downloading them is generally infringement. Homebrew released by its authors, freely licensed programs, emulator test programs, and dumps of games you own, where your country's law allows making them, are the legitimate options." },
    ],
  };

  // Which banks each of the book's fourteen chapters may draw follow-ups from.
  ...
  function lessonStudyTerms(root) {
    const terms = new Map();
    const lesson = root.closest(".sl-markdown-content") || document.body;
