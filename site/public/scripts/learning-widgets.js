(function () {
  ...
  const REVIEW_BANKS = {
    1: [
      ...
      { prompt: "Why do an engine's console variables and debug commands make good anchors for an experiment?", options: ["Toggling one gives a clean cause, so whatever changes in memory at the same moment is connected to that feature", "They disable anti-cheat", "They are the only way to read memory", "They make every address permanent"], answer: 0, explanation: "A controlled toggle is the experiment Chapter 1 keeps asking for: one change, one observed effect. Engines often ship these tools because their developers needed them." },
