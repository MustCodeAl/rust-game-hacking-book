...
<div class="academy-glossary">
  ...
  <section class="glossary-group" aria-labelledby="glossary-a">
    ...
    <dl class="glossary-list">
      ...
      <dt id="term-anti-rollback"><dfn>Anti-rollback</dfn></dt>
      <dd>A check that refuses genuinely signed but older software by comparing each image's security version with a minimum kept in fuses. It stops anyone reinstalling a version with a known security hole.</dd>
      ...
      <dt id="term-ascii"><dfn>ASCII</dfn></dt>
      <dd>The standard table that gives English letters, digits, and common symbols the numbers 0 to 127; for example <code>0x41</code> is <code>A</code>. Bytes from <code>0x20</code> (a space) to <code>0x7E</code> are printable.</dd>
      ...
      <dt id="term-authoritative-state"><dfn>Authoritative state</dfn></dt>
      <dd>The one copy of a value whose contents decide what happens next, such as the simulation's entity object or, in a multiplayer game, the server's record. Checks that protect a value must run against this copy, not a cached or displayed one.</dd>
    </dl>
  </section>
  ...
  <section class="glossary-group" aria-labelledby="glossary-b">
    ...
    <dl class="glossary-list">
      ...
      <dt id="term-behavior-tree"><dfn>Behavior tree</dfn></dt>
      <dd>A tree of conditions and actions that an NPC evaluates from the root every tick. A selector node tries its children until one succeeds; a sequence node runs its children until one fails.</dd>
      ...
      <dt id="term-boot-rom"><dfn>Boot ROM</dfn></dt>
      <dd>The first code a chip runs at power-on, written into the chip when it is made and unchangeable afterwards. In secure boot it checks the first stage that can be updated.</dd>
      <dt id="term-border"><dfn>Border</dfn></dt>
      <dd>In Lua, a key whose value is present while the next key's value is missing. The length operator <code>#</code> may return any border, so a table with a hole in its sequence has no single length.</dd>
      <dt id="term-boundary-scan"><dfn>Boundary scan</dfn></dt>
      <dd>A test method in which a cell between each chip pin and the chip's logic can capture the pin's level or drive a chosen level. The cells form a shift register, so every connection on a board can be tested through one serial chain.</dd>
      ...
      <dt id="term-bug-check"><dfn>Bug check</dfn> <span class="glossary-alias">Blue screen, stop error</span></dt>
      <dd>Windows deliberately halting the whole machine after a kernel-mode failure it cannot safely recover from, showing a stop code and writing a crash dump.</dd>
  ...
  <section class="glossary-group" aria-labelledby="glossary-c">
    ...
    <dl class="glossary-list">
      ...
      <dt id="term-chain-of-trust"><dfn>Chain of trust</dfn></dt>
      <dd>The sequence of secure-boot stages, each trusted only because the stage before it checked its signature. It starts at a root of trust that nothing checks, because nothing can change it.</dd>
      <dt id="term-cheat-code"><dfn>Cheat code</dfn></dt>
      <dd>For an old console game, an address and a value: either a byte substituted when the game reads that address, or a value written to RAM once every frame. It works because those games keep their variables at fixed addresses.</dd>
      ...
      <dt id="term-clock-cycle"><dfn>Clock cycle</dfn> <span class="glossary-alias">Cycle</span></dt>
      <dd>One tick of a processor's clock. Emulators count the cycles each instruction takes, so the emulated CPU, graphics, and sound stay in step as they did in the original hardware.</dd>
      ...
      <dt id="term-context-switch"><dfn>Context switch</dfn></dt>
      <dd>Saving one thread's registers and loading another's so a core can run a different thread. Switching to a thread of another process also loads that process's page tables into <code>cr3</code>.</dd>
      ...
      <dt id="term-ioctl"><dfn>Control code</dfn> <span class="glossary-alias">IOCTL</span></dt>
      <dd>A 32-bit number sent with <code>DeviceIoControl</code> that packs a device type, the access the caller's handle must hold, a function number, and a buffer method.</dd>
      ...
      <dd>Confirming a field with independent checks that all agree, such as the instructions that access it, how its value changes when the game changes, the object it belongs to, and its range relative to other fields.</dd>
  ...
  <section class="glossary-group" aria-labelledby="glossary-d">
    ...
    <dl class="glossary-list">
      ...
      <dt id="term-dpc"><dfn>Deferred procedure call</dfn> <span class="glossary-alias">DPC</span></dt>
      <dd>Driver code queued by an interrupt service routine to run shortly afterward at a lower priority, where it finishes the device work and completes waiting requests.</dd>
      ...
      <dt id="term-device-register"><dfn>Device register</dfn></dt>
      <dd>A small storage location inside a device that software reads to learn the device's state or writes to command it. Unlike RAM, reading or writing a register can have side effects, such as removing data from a queue.</dd>
      <dt id="term-differential-comparison"><dfn>Differential comparison</dfn></dt>
      <dd>Running the same code on two inputs that differ in one known way and comparing the outputs. For an XOR-based value transform the key cancels out, so the comparison exposes the rest of the transform without knowing the key.</dd>
      ...
      <dt id="term-dot-product"><dfn>Dot product</dfn></dt>
      <dd>Two vectors' matching parts multiplied and added: <code>a.x × b.x + a.y × b.y</code>. For two unit vectors it equals the cosine of the angle between them.</dd>
      ...
      <dt id="term-driver"><dfn>Driver</dfn> <span class="glossary-alias">Device driver</span></dt>
      ...
      <dt id="term-driver-signing"><dfn>Driver signing</dfn></dt>
      <dd>The rule that 64-bit Windows loads kernel-mode drivers only with a valid digital signature; since Windows 10 version 1607, with Secure Boot on, new drivers must be signed through Microsoft's hardware developer program.</dd>
      <dt id="term-driver-stack"><dfn>Driver stack</dfn></dt>
      <dd>The drivers layered on one device: optional filter drivers that watch or adjust requests, the function driver that operates the device, and the bus driver for the bus it is plugged into.</dd>
      <dt id="term-dynamic-recompilation"><dfn>Dynamic recompilation</dfn> <span class="glossary-alias">Dynarec, JIT</span></dt>
      <dd>Translating guest machine code into host machine code while the program runs, one block at a time, and reusing the stored translation. It is much faster than interpreting, but it needs memory that is written and then executed, and it must discard translations when the guest rewrites its code.</dd>
    </dl>
  </section>
  ...
  <section class="glossary-group" aria-labelledby="glossary-e">
    ...
    <dl class="glossary-list">
      ...
      <dt id="term-efuse"><dfn>eFuse</dfn> <span class="glossary-alias">One-time programmable fuse</span></dt>
      <dd>A bit on a chip that can be changed once, from 0 to 1, and never back. Chips keep settings that must not be undone in fuses, such as a locked debug port, a key hash, or an anti-rollback counter.</dd>
      <dt id="term-emulator"><dfn>Emulator</dfn></dt>
      <dd>A program that imitates another computer's hardware closely enough that the other computer's software runs on it unchanged. The imitated machine and its software are the guest; the computer running the emulator is the host.</dd>
      ...
      <dt id="term-exponent-bias"><dfn>Exponent bias</dfn> <span class="glossary-alias">Bias</span></dt>
      <dd>A fixed number added to a floating-point value's real exponent so the stored exponent never needs a minus sign. An <code>f32</code> stores the exponent plus 127, so a stored 130 means 2³ and a stored 126 means 2⁻¹.</dd>
  ...
  <section class="glossary-group" aria-labelledby="glossary-f">
    ...
    <dl class="glossary-list">
      ...
      <dt id="term-floating-point"><dfn>Floating point</dfn> <span class="glossary-alias">f32 / f64</span></dt>
      <dd>A way to store numbers with a fractional part as a sign, an exponent, and a fraction, like scientific notation in binary. An <code>f32</code> uses 1 + 8 + 23 bits; an <code>f64</code> uses 1 + 11 + 52. Most values are stored as the nearest representable number, not exactly.</dd>
      ...
      <dd>Whether a check result still describes the state being used now. A correct result can become stale after an object generation, command, frame, or thread-visible state changes.</dd>
  ...
  <section class="glossary-group" aria-labelledby="glossary-g">
    ...
    <dl class="glossary-list">
      ...
      <dt id="term-general-protection-fault"><dfn>General-protection fault</dfn></dt>
      <dd>The processor exception, vector 13, raised when code breaks a protection rule, such as running a privileged instruction like <code>hlt</code> or <code>wrmsr</code> in ring 3.</dd>
      ...
      <dt id="term-guest"><dfn>Guest</dfn></dt>
      <dd>The operating system and programs running inside a virtual machine.</dd>
      <dt id="term-guest-physical-address"><dfn>Guest physical address</dfn></dt>
      <dd>An address a guest believes is physical. The hypervisor's second-stage tables translate it into the host physical address where the byte really is.</dd>
    </dl>
  </section>
  ...
  <section class="glossary-group" aria-labelledby="glossary-h">
    ...
    <dl class="glossary-list">
      ...
      <dt id="term-hardware-virtualization"><dfn>Hardware virtualization</dfn> <span class="glossary-alias">Intel VT-x, AMD-V</span></dt>
      <dd>Processor support that runs a guest in a separate mode with its own rings 0 to 3, at full speed, until an action the hypervisor listed causes a VM exit.</dd>
      ...
      <dt id="term-hex-editor"><dfn>Hex editor</dfn></dt>
      <dd>A program that shows and edits a file as numbered bytes, each written as two hexadecimal digits, beside an offset column and a character column. Unlike a text editor, it never converts line endings or text encodings when it saves.</dd>
      ...
      <dt id="term-hid"><dfn>HID</dfn> <span class="glossary-alias">Human interface device</span></dt>
      <dd>The USB standard for keyboards, mice, and game controllers. A HID device sends reports; a boot-format keyboard report is 8 bytes holding modifier bits and up to six held keys.</dd>
      <dt id="term-hidden-bit"><dfn>Hidden bit</dfn></dt>
      <dd>The leading 1 of a normal floating-point number. It is always 1, so it is not stored; the reader adds it back, which gives an <code>f32</code> 24 bits of precision from 23 stored fraction bits.</dd>
      <dt id="term-homebrew"><dfn>Homebrew</dfn></dt>
      <dd>Software written by hobbyists for a platform without the platform holder's licence, such as games, tools, and emulators. Its legal position depends on the country and on how the platform's protections are treated.</dd>
      ...
      <dt id="term-host"><dfn>Host</dfn></dt>
      <dd>The real computer, and its operating system if there is one, on which a hypervisor runs virtual machines.</dd>
      <dt id="term-hypervisor"><dfn>Hypervisor</dfn> <span class="glossary-alias">Virtual machine monitor</span></dt>
      <dd>Software that creates virtual machines and shares the real processors, memory, and devices among them.</dd>
    </dl>
  </section>
  ...
  <section class="glossary-group" aria-labelledby="glossary-i">
    ...
    <dl class="glossary-list">
      <dt id="term-io-manager"><dfn>I/O manager</dfn></dt>
      <dd>The part of the Windows kernel that runs every device request: it checks the caller's handle, builds an I/O request packet, sends it down the driver stack, and returns the result.</dd>
      <dt id="term-irp"><dfn>I/O request packet</dfn> <span class="glossary-alias">IRP</span></dt>
      <dd>The kernel data structure that describes one device request, such as a read or a control code, with its parameters and buffers. A driver completes it with a status and a byte count.</dd>
      ...
      <dt id="term-idcode"><dfn>IDCODE</dfn></dt>
      <dd>A 32-bit JTAG register that identifies a chip by version, part number, and JEDEC manufacturer code. Its bit 0 is always 1, which tells it apart from a bypass register's captured 0.</dd>
      ...
      <dt id="term-interpreter"><dfn>Interpreter</dfn></dt>
      <dd>A program that runs another program one instruction or statement at a time, fetching, decoding, and carrying out each in a loop, without first translating it into native machine code.</dd>
      <dt id="term-interrupt"><dfn>Interrupt</dfn></dt>
      <dd>A signal from hardware, such as a timer tick or a device with data ready, that makes the processor pause between two instructions and run the handler the kernel registered for it.</dd>
      <dt id="term-interrupt-descriptor-table"><dfn>Interrupt descriptor table</dfn> <span class="glossary-alias">IDT</span></dt>
      <dd>The table of up to 256 entries, filled in by the kernel at boot, that tells the processor which handler to run for each exception or interrupt vector. Vector 3 is the <code>int3</code> breakpoint and vector 14 the page fault.</dd>
      <dt id="term-isr"><dfn>Interrupt service routine</dfn> <span class="glossary-alias">ISR</span></dt>
      <dd>A driver's first response to its device's interrupt. It runs while other work on the core waits, so it only confirms the interrupt, acknowledges it, and queues a deferred procedure call.</dd>
  ...
  <section class="glossary-group" aria-labelledby="glossary-j">
    ...
    <dl class="glossary-list">
      <dt id="term-jtag"><dfn>JTAG</dfn> <span class="glossary-alias">IEEE 1149.1</span></dt>
      <dd>A standard test port built into chips, using the signals TCK, TMS, TDI, and TDO. It was designed to test solder joints on assembled boards through boundary scan, and it is also the usual way to reach a chip's on-chip debug logic.</dd>
  ...
  <section class="glossary-group" aria-labelledby="glossary-l">
    ...
    <dl class="glossary-list">
      ...
      <dt id="term-length-prefixed-string"><dfn>Length-prefixed string</dfn></dt>
      <dd>Text stored as a count followed by exactly that many bytes, with no end marker. Every field after it moves when the length changes, so a parser checks the count against the real data size before trusting it.</dd>
      ...
      <dt id="term-line-of-sight"><dfn>Line of sight</dfn></dt>
      <dd>Whether the straight segment between two points is free of solid geometry. Games test it with a ray cast against collision shapes.</dd>
  ...
  <section class="glossary-group" aria-labelledby="glossary-m">
    ...
    <dl class="glossary-list">
      ...
      <dt id="term-magic-number"><dfn>Magic number</dfn> <span class="glossary-alias">File signature</span></dt>
      <dd>A fixed byte pattern at the start of a file that identifies its format, such as <code>47 53 41 56</code> (<code>GSAV</code>) or the eight-byte PNG signature. A loader checks it before trusting any other field.</dd>
      ...
      <dt id="term-memory-integrity"><dfn>Memory integrity</dfn> <span class="glossary-alias">HVCI</span></dt>
      <dd>The VBS feature that runs kernel code-integrity checks in VTL1 and uses second-stage page permissions, so kernel pages become executable only after passing those checks and are never writable while executable.</dd>
      <dt id="term-memory-map"><dfn>Memory map</dfn></dt>
      <dd>The table of what answers at each address: RAM, ROM, or a device's registers. On a console it is the same on every unit and every run.</dd>
      ...
      <dt id="term-mmio"><dfn>Memory-mapped I/O</dfn> <span class="glossary-alias">MMIO</span></dt>
      <dd>Reaching a device through addresses: loads and stores to those addresses go to the device’s registers instead of RAM. The kernel maps such ranges as uncacheable, and an emulator’s bus function imitates the same routing in software.</dd>
  ...
  <section class="glossary-group" aria-labelledby="glossary-n">
    ...
    <dl class="glossary-list">
      ...
      <dt id="term-navigation-mesh"><dfn>Navigation mesh</dfn> <span class="glossary-alias">navmesh</span></dt>
      <dd>A set of polygons covering the floor an NPC can walk on. Pathfinding such as A* runs over it to plan routes around obstacles.</dd>
      ...
      <dt id="term-npc"><dfn>Non-player character</dfn> <span class="glossary-alias">NPC</span></dt>
      <dd>A character the game controls rather than a person, such as a guard, a shopkeeper, or an enemy. The code that picks what it does next is usually called its AI.</dd>
      ...
      <dt id="term-ntstatus"><dfn>NTSTATUS</dfn></dt>
      <dd>The 32-bit status code returned by native Windows functions and drivers. Its top two bits give the severity (00 success, 01 information, 10 warning, 11 error), so <code>0xC0000022</code>, access denied, is an error.</dd>
  ...
  <section class="glossary-group" aria-labelledby="glossary-p">
    ...
    <dl class="glossary-list">
      ...
      <dt id="term-page-frame"><dfn>Page frame</dfn></dt>
      <dd>A page-sized block of physical memory. A page-table entry maps one virtual page to one frame.</dd>
      ...
      <dt id="term-paravirtualization"><dfn>Paravirtualization</dfn></dt>
      <dd>Changing a guest operating system or its drivers so that they ask the hypervisor directly instead of running instructions or touching devices that would have to be trapped.</dd>
      ...
      <dt id="term-place-value"><dfn>Place value</dfn></dt>
      <dd>What a digit is worth because of its position. In binary each position is worth twice the one to its right, so the positions of a byte are worth 128, 64, 32, 16, 8, 4, 2, and 1; position <em>n</em> is worth 2ⁿ.</dd>
      ...
      <dt id="term-pointee-type"><dfn>Pointee type</dfn></dt>
      <dd>The type of the data a pointer leads to, such as <code>f32</code> in <code>*const f32</code>. It decides how many bytes are read at the destination, how they are interpreted, and how far pointer arithmetic steps. It does not change the pointer's own size, which the process fixes at 4 or 8 bytes.</dd>
      ...
      <dt id="term-program-counter"><dfn>Program counter</dfn> <span class="glossary-alias">PC</span></dt>
      <dd>The register holding the address of the next instruction to fetch. x86 calls it the instruction pointer, <code>EIP</code> or <code>RIP</code>.</dd>
      ...
      <dt id="term-protection-ring"><dfn>Protection ring</dfn> <span class="glossary-alias">Ring, privilege level</span></dt>
      <dd>One of the processor's privilege levels, numbered 0 to 3 on x86. Windows runs the kernel and drivers in ring 0 and ordinary programs in ring 3; the current level is the lowest two bits of the <code>cs</code> register.</dd>
  ...
  <section class="glossary-group" aria-labelledby="glossary-r">
    ...
    <dl class="glossary-list">
      ...
      <dt id="term-remote-attestation"><dfn>Remote attestation</dfn></dt>
      <dd>A device proving to a remote service what software it booted. A key that software cannot read signs the service's nonce together with hashes recorded by each boot stage, and the service compares them with known-good values.</dd>
      ...
      <dt id="term-root-of-trust"><dfn>Root of trust</dfn></dt>
      <dd>The first link of a chain of trust, trusted without being checked because it cannot be changed, such as a boot ROM with a key hash in fuses. A flaw in it cannot be fixed on devices already sold.</dd>
  ...
  <section class="glossary-group" aria-labelledby="glossary-s">
    ...
    <dl class="glossary-list">
      <dt id="term-save-state"><dfn>Save state</dfn></dt>
      <dd>A copy of an emulated machine's entire state, including registers, RAM, I/O registers, and timing position, that can be loaded to resume from exactly that instant. Unlike a game's own save file, it works for any game at any moment.</dd>
      <dt id="term-scan-chain"><dfn>Scan chain</dfn></dt>
      <dd>Several chips' JTAG ports wired in series, each chip's TDO feeding the next chip's TDI, so a debugger sees one long shift register. With every chip in BYPASS, each adds exactly one bit.</dd>
      <dt id="term-scan-code"><dfn>Scan code</dfn></dt>
      <dd>A number that identifies a physical key position rather than the character it produces. Windows uses set-1 scan codes, in which the A key is <code>0x1E</code>.</dd>
      <dt id="term-scheduler"><dfn>Scheduler</dfn></dt>
      <dd>The part of the kernel that decides which ready thread runs on each processor core, using timer interrupts to take the processor back.</dd>
      <dt id="term-slat"><dfn>Second-level address translation</dfn> <span class="glossary-alias">SLAT, EPT, NPT</span></dt>
      <dd>Hardware two-stage translation: the guest's page tables turn guest virtual addresses into guest physical addresses, and tables owned by the hypervisor turn those into host physical addresses.</dd>
      <dt id="term-secure-boot"><dfn>Secure boot</dfn> <span class="glossary-alias">Secure Boot</span></dt>
      <dd>A start-up process in which each stage checks the digital signature of the next stage before running it, and refuses anything that fails. On PCs, UEFI Secure Boot lets only signed boot loaders start; consoles build the same chain from a boot ROM.</dd>
      <dt id="term-secure-debug-authentication"><dfn>Secure debug authentication</dfn></dt>
      <dd>Keeping a debug port locked until the debugger returns a signature, made with a key only the maker holds, over a fresh nonce and the chip's unique ID. Authorised engineers can debug a returned unit without that answer opening any other device.</dd>
      <dt id="term-security-processor"><dfn>Security processor</dfn></dt>
      <dd>A small separate processor on a console or PC chip that starts before the main cores, holds the device's keys, and performs boot checks and decryption. The main cores and their software never see the keys.</dd>
      ...
      <dt id="term-server-authority"><dfn>Server authority</dfn> <span class="glossary-alias">Authoritative server</span></dt>
      <dd>A design in which the server's record is the authoritative state and every client request is checked against it, because anything a client computed can be changed before it is sent.</dd>
      ...
      <dt id="term-shift-register"><dfn>Shift register</dfn></dt>
      <dd>A row of one-bit storage cells that all move one place along on each clock tick: a new bit enters at one end and the bit at the other end leaves. It turns many bits into a stream on a single wire.</dd>
      ...
      <dt id="term-significand"><dfn>Significand</dfn> <span class="glossary-alias">Mantissa</span></dt>
      <dd>The digits of a floating-point number with the hidden leading 1 put back. For 10.0 = 1.25 × 2³ the significand is 1.25; the stored fraction field holds only the .25 part.</dd>
      ...
      <dt id="term-swd"><dfn>SWD</dfn> <span class="glossary-alias">Serial Wire Debug</span></dt>
      <dd>Arm's two-wire alternative to JTAG, using the clock SWCLK and the shared data wire SWDIO. It sends 8-bit request packets instead of stepping through a state machine, and reaches the same debug port.</dd>
      ...
      <dt id="term-soc"><dfn>System on a chip</dfn> <span class="glossary-alias">SoC</span></dt>
      <dd>A single chip that holds most of a computer: CPU cores, a GPU, memory controllers, and often a security processor. Consoles and phones are built around one.</dd>
      <dt id="term-system-service-number"><dfn>System service number</dfn></dt>
      <dd>The number an <code>ntdll.dll</code> stub places in <code>eax</code> before <code>syscall</code> to choose which kernel routine runs. The numbers are internal to Windows and change between builds.</dd>
    </dl>
  </section>
  ...
  <section class="glossary-group" aria-labelledby="glossary-t">
    ...
    <dl class="glossary-list">
      <dt id="term-tap-controller"><dfn>TAP controller</dfn> <span class="glossary-alias">Test Access Port controller</span></dt>
      <dd>The 16-state machine inside every JTAG chip. At each rising edge of TCK the level of TMS picks one of two next states, and five clocks with TMS at 1 reach Test-Logic-Reset from any state.</dd>
      ...
      <dt id="term-tick"><dfn>Tick</dfn></dt>
      <dd>One fixed step of a game's simulation. Every tick advances the world by the same slice of time, so behavior counted in ticks does not depend on the frame rate.</dd>
      ...
      <dt id="term-tlb"><dfn>Translation lookaside buffer</dfn> <span class="glossary-alias">TLB</span></dt>
      <dd>A small processor cache of recent address translations, so repeated accesses to the same pages skip the page-table walk.</dd>
      <dt id="term-trap-and-emulate"><dfn>Trap and emulate</dfn></dt>
      <dd>Running a guest kernel without privilege and letting the hypervisor catch and imitate each sensitive instruction. It needs every sensitive instruction to fault, which older x86 processors did not guarantee.</dd>
      ...
      <dt id="term-twos-complement"><dfn>Two's complement</dfn></dt>
      <dd>The usual way to store signed whole numbers: the leftmost bit's place value is negative (−128 in a byte), and every other bit keeps its normal value. So <code>0xFF</code> is −1 as an <code>i8</code> and 255 as a <code>u8</code>, and the CPU adds both with the same circuit.</dd>
      <dt id="term-type"><dfn>Type</dfn></dt>
      <dd>How many bytes a value occupies, what its bits mean, and which operations are allowed on it. Raw bytes do not reveal their type; the instructions that read and use them do.</dd>
      <dt id="term-type-1-hypervisor"><dfn>Type 1 hypervisor</dfn></dt>
      <dd>A hypervisor that runs directly on the hardware, with every operating system above it, such as Hyper-V, Xen, or VMware ESXi.</dd>
      <dt id="term-type-2-hypervisor"><dfn>Type 2 hypervisor</dfn></dt>
      <dd>A hypervisor that runs as an application on a host operating system and relies on it for devices and scheduling, such as VirtualBox or VMware Workstation.</dd>
    </dl>
  </section>
  ...
  <section class="glossary-group" aria-labelledby="glossary-u">
    ...
    <dl class="glossary-list">
      ...
      <dt id="term-unified-memory"><dfn>Unified memory</dfn></dt>
      <dd>One pool of memory shared by the CPU and the GPU, so both use the same bytes without copying data between separate memories. Consoles use it; most gaming PCs give the GPU its own video RAM.</dd>
      <dt id="term-unit-vector"><dfn>Unit vector</dfn></dt>
      <dd>A vector of length 1, used to store a direction. Dividing a vector by its own length produces one.</dd>
      ...
      <dt id="term-utility-ai"><dfn>Utility AI</dfn> <span class="glossary-alias">utility scoring</span></dt>
      <dd>A way of choosing an action by giving every option a score computed from the current situation and running the highest-scoring one.</dd>
    </dl>
  </section>
  ...
  <section class="glossary-group" aria-labelledby="glossary-v">
    ...
    <dl class="glossary-list">
      ...
      <dt id="term-view-cone"><dfn>View cone</dfn></dt>
      <dd>The range of directions an NPC can see: every direction within a set angle of its facing. Code usually tests it by comparing a dot product with the cosine of that angle.</dd>
      ...
      <dd>A software execution engine with its own instruction set and runtime state. Lua’s VM executes Lua bytecode; a hardware virtual machine instead emulates or partitions an entire computer. A system virtual machine, run by a hypervisor, instead presents a whole computer with the host’s own instruction set, so a guest operating system can run on it unmodified.</dd>
      ...
      <dt id="term-virtual-key-code"><dfn>Virtual-key code</dfn></dt>
      <dd>The number Windows gives a key in window messages and key-state functions, produced from the scan code by the active keyboard layout. <code>VK_A</code> is <code>0x41</code>.</dd>
      <dt id="term-vbs"><dfn>Virtualization-based security</dfn> <span class="glossary-alias">VBS</span></dt>
      <dd>The Windows feature that runs Windows above its own hypervisor in two virtual trust levels, so a secure world in VTL1 can protect memory and secrets from the normal kernel in VTL0.</dd>
      <dt id="term-vm-escape"><dfn>VM escape</dfn></dt>
      <dd>A hypervisor bug that lets code in a guest run on the host, outside the guest's isolation and beyond any snapshot's reach.</dd>
      <dt id="term-vm-exit"><dfn>VM exit</dfn></dt>
      <dd>The processor stopping a guest, saving its state, and running the hypervisor's handler with a reason code, for example after <code>cpuid</code> or an access to a guest physical page with no second-stage mapping.</dd>
      <dt id="term-vm-snapshot"><dfn>VM snapshot</dfn></dt>
      <dd>A saved state of a whole virtual machine: its disk, frozen with later writes going to a delta file, and for a running machine its memory, processor registers, and device state.</dd>
      ...
      <dt id="term-vulnerable-driver-blocklist"><dfn>Vulnerable driver blocklist</dfn></dt>
      <dd>Microsoft's list of signed drivers with known exploitable bugs, which Windows refuses to load. It is on by default since the Windows 11 2022 update and enforced whenever memory integrity is on.</dd>
    </dl>
  </section>
  ...
  <footer class="academy-glossary__footer">
    ...
    <a href="../contents/">Browse all lessons <span aria-hidden="true">→</span></a>
  </footer>
</div>
