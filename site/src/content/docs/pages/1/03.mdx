...
Zero needs its own pattern because it has no leading 1 to hide. That is also
what happens to the bytes from the start of this lesson. Read `64 00 00 00` as
an `f32` and, most significant first, it is `0x00000064`: sign 0, exponent field
