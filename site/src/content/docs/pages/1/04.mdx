...
description: "See how game loops, objects, fields, collections, identities, rules, and display copies organize a changing world."
chapter: "1.4"
minutes: 18
author: "attilathedud"
date: "2026-07-30"
sidebar:
  order: 4
  label: "1.4 How Games Organize Data"
---
...
These layouts store the same data in different orders, and the instructions
that read the values show which one a game uses.
...
only thing that reveals this is not the object the holder meant. A tool that
keeps a reference to a game object has the same problem, so it needs to store
something that proves which object it is, such as the generation, not only
