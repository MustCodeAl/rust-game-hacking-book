...
description: "Learn the programming vocabulary the rest of the book assumes — data, types, state, algorithms, data structures, functions, event-driven programs, abstractions, APIs, ABIs, parsing, concurrency, and invariants."
chapter: "1.5"
minutes: 26
author: "attilathedud"
date: "2026-07-30"
sidebar:
  order: 5
  label: "1.5 Programming Fundamentals"
---
...
express it, and *exact* matters: the computer does what the steps say, not what
you meant. If step 3 compares only every fourth byte, a value stored at an odd
address is never found, and nothing warns you.
...
[Lesson 3.5](/pages/3/05/#linked-structures-leave-different-clues) shows what
lists, trees, and hash tables look like inside a running game.
...
A bug whose result depends on which task happens to run first is called a **race
condition**. The classic one loses an update. A player has 1,000 gold, and on two
threads a quest reward adds 500 while a purchase subtracts 300. Both read 1,000.
One writes 1,500, the other writes 700 last, and the player ends with 700 instead
of 1,200. Nothing crashes, and most runs happen in a lucky order, which is why
races pass testing and then fail for someone.
[Lesson 10.6](/pages/10/06/#a-race-that-loses-an-update) walks through it step
by step.

Later lessons answer these with snapshots, locks (which make the second update
wait until the first has written), atomics (single
...
Invariants help reverse engineering because they tie fields to each other. A
value of 80 on its own matches many unrelated addresses, but a candidate
`health` that stays at or below a neighboring `max_health` through every hit
and every heal is very unlikely to be a coincidence.
