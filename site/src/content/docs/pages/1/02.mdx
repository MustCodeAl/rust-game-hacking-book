...
description: "Follow a game from its file on disk to a running process that reads input, updates memory, and draws frames."
chapter: "1.2"
minutes: 16
author: "attilathedud"
date: "2026-07-30"
sidebar:
  order: 2
  label: "1.2 How a Computer Runs a Game"
---
...
delayed, filtered, or supplied by something other than a real key press.

So every result, and every bug, belongs to a particular layer. If a window
message arrives but the game ignores it, the input reached the window-message
layer; that does not prove the game's device-input system accepted it. Later
lessons follow these boundaries directly.
