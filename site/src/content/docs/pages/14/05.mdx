...
Second, the word "shared" in the memory column. In a gaming PC, the CPU uses
system RAM on the motherboard, and the graphics card carries its own video RAM.
Anything the GPU needs, such as the textures and vertex buffers of
[Chapter 5](/pages/5/01/), is copied from one to the other across the PCI
Express bus. In a console, the CPU and the GPU use one pool of memory: the CPU
writes a buffer, and the GPU reads the very same bytes. This design is called
**unified memory**.
...
it from graphics memory. The PlayStation 5's memory bus is 256 bits wide, and
each of those 256 wires carries 14 billion bits per second. That is
256 × 14 = 3,584 billion bits per second, and dividing by 8 bits per byte gives
448 billion bytes, or 448 GB, every second, available to the CPU and GPU alike.

The third difference is not in the table because it is the table: every
PlayStation 5 has the same chip, the same memory, and the same clock speeds. A PC
game has to run on thousands of combinations of parts, while a console game is
tuned for exactly one. At 60 frames per second, each frame gets
1,000 ms ÷ 60 ≈ 16.7 ms for all of its work: game logic, physics, and rendering
([Lesson 1.10](/pages/1/10/)). A measurement on one console holds for every
console, so developers can spend that budget almost to the last millisecond.
They can also compile their shaders ahead of time for the one GPU and ship the
results on the disc, which [Lesson 14.6](/pages/14/06/) will show is a luxury
emulators do not have.

Same instruction set does not mean same platform. A console game's machine code
is x86-64 or Arm like a PC's, but it calls the console's own operating system
and graphics interfaces: Xbox games use a console version of Direct3D 12, and
PlayStation games use Sony's own graphics interfaces. That is why moving a
console game to PC is still porting work.
...
game partition that runs the current game, and a Windows-based partition that
runs the home screen and apps. Consoles differ in the details, but the same
shape appears again and again:
...
- **Predictable timing.** The game partition gets CPU cores and memory reserved
  for it, so a download running in the background cannot steal time from a
  frame. With 16.7 ms per frame, a background task that takes 2 ms at the wrong
  moment is a visible stutter.
- **Containment.** A bug in a game, such as a crash caused by a malformed save
  ...
  account tokens and purchase records live, is simply not mapped into the game's
  address space.
...
flowchart TD
    ...
    R -->|"checks the bootloader's key against the fused hash,<br/>then its signature and its security version"| B["bootloader"]
    B -->|"checks the signatures of the<br/>hypervisor and system kernels"| H["hypervisor and system kernels"]
    H -->|"checks each system component"| S["system software:<br/>home screen, store, network"]
    S -->|"checks the game package's signature<br/>and licence, then decrypts it"| G["game"]
...
it is manufactured. No software, update, or owner can change it. It is the
...
The boot ROM needs a public key to check the first signature, and that key has
to be as unchangeable as the ROM. Chips keep such values in **eFuses**, one-time
programmable bits blown at the factory ([Lesson 14.4](/pages/14/04/)). Fuses are
expensive, since each one is a physical structure on the chip. A 2,048-bit RSA
public key would need 2,048 of them, so chips store a SHA-256 hash of the key
instead: 256 bits, so 256 fuses. The key itself travels in flash storage
...
earlier, already-checked stage vouches for. A bug in any later stage can be fixed
by an update, because the stage before it will verify the replacement's
signature like any other. A bug in the boot ROM cannot be fixed on consoles
already sold, because nothing checks the ROM and nothing can rewrite it. Only a
new revision of the chip fixes it. This has happened to at least one console
family, whose early units remain modifiable for that reason alone.
...
still genuinely signed. Without another check, anyone could reinstall version 3
and use its hole.
...
unblown, so the minimum only ever rises. A bank of eight fuses can count only to
8, which is why makers raise the security version only when an update fixes a
security flaw, not with every update: each increase spends a fuse forever.
...
updates, apps, and every game, must carry the platform holder's signature, and
the system software checks it before launching anything. Unsigned code simply
does not run. Compare Windows, where an Authenticode signature
...
rule matters again in Lesson 14.6, because an emulator's fastest technique needs
...
defends its owner against other people: malware, other users, network attackers.
The owner is the administrator and may switch any of it off. A console's security
also defends the platform holder and game publishers against the owner: against
copying games, against cheating in online play, and against running unapproved
software. So a console is built so that its owner cannot switch the protections
off. Trust starts in silicon, keys never leave the security processor, debug
ports are locked with fuses ([Lesson 14.4](/pages/14/04/)), and old software
...
using a key unique to that console that was fused at the factory and that no
software can read. The service checks the signature and compares the
measurements with the values genuine software produces. Console makers do not
publish their exact protocols, but PCs with a TPM security chip use the same
idea, called measured boot.
...
software it is not. Why ban it rather than just ignore the report? A modified
console can run cheats in matches with other people, run pirated games, and send
the service messages that genuine software never would, so nothing it says can
be trusted. Banning the console by its unique ID, and sometimes the account,
protects other players and the store. The ban is the service's decision under the
terms of service the owner accepted, not a technical malfunction.
