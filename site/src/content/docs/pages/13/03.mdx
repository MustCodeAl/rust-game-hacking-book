...
description: "Replace a vague list of debugger signals with an observable state machine, explicit decisions, false-positive tests, and game invariants that do not depend on one environmental clue."
chapter: "13.3"
minutes: 45
author: "attilathedud"
date: "2026-08-31"
sidebar:
  order: 3
  label: "13.3 Model Anti-Debug Behavior as a State Machine"
---
## Prerequisites
...
program can take the signal from several genuinely different sources, and they
differ in what they prove, which is why a signal and the assessment drawn from
it are kept apart.
...
None of them establishes that a game rule was broken. That is why environment
signals stay separate from validating game state, and why the invariant at the
top belongs at the effect boundary rather than anywhere in this list.
...
4. **across one controlled change**, such as the same fixture run with a
   breakpoint set, to see which transitions differ;
