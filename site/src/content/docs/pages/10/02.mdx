...
description: "Create and verify a SHA-256 manifest so RVAs, byte patterns, mods, and patches stay tied to exact files."
chapter: "10.2"
minutes: 30
author: "attilathedud"
date: "2026-07-30"
sidebar:
  order: 2
  label: "10.2 Prove Which Game Build You Have"
---
...
That matters because much of what this course builds depends on exact bytes:
...
Several different facts identify a build, and each one answers a separate
question:

| Fact | Question it answers | Important limit |
|---|---|---|
...
The complete implementation—including creation, parsing, verification, checked size arithmetic, and non-overwriting output—is [`integrity_manifest.rs`](/windows-labs/src/bin/integrity_manifest.rs).
...
Do not update `clean-install.manifest` until a changed file looks clean again. Keep the original baseline, and create a second, reviewed manifest after the intended change, so each known state has its own list of digests.

The Urban Terror texture lab from Lesson 9.3 fits this pattern exactly. Before the tool runs, the archive's digest belongs to the clean state. The tool keeps the untouched original as `zUrT43_001.pk3.gha-backup`, so that backup must hash to the same clean digest; if it does not, the original was not preserved. The rebuilt `zUrT43_001.pk3` has a new digest, which belongs to the modded state.
...
Each check covers a gap the others leave. The on-disk hash cannot tell which copy of the file the process actually mapped, or whether anything changed the bytes in memory after loading; the original-byte check at the final address can. A patch should go ahead only when every check passes.
