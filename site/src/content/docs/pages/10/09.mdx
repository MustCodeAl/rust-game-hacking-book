...
description: "Work out whether a game is native, Mono, IL2CPP, .NET, or Java before choosing a method, because the runtime decides whether addresses, offsets, and names mean anything."
chapter: "10.9"
minutes: 26
author: "attilathedud"
date: "2026-09-03"
sidebar:
  order: 9
  label: "10.9 Identify the Engine and Runtime First"
---
...
the executable you inspected. The folder shows what is installed; the module
list shows what the running process actually mapped.
...
That is the opposite of the Unity Mono case above. Both are "managed" and
"garbage collected," and only one of them moves objects. The word "managed" is
not what determines whether an address is durable — the specific collector is.
...
long-lived static, or a re-lookup by name each time. Lesson 1.4 met the same
problem with slot-and-generation handles: when storage can be reused or
relocated, a program has to hold something that identifies the object itself,
not the place it happened to occupy.
...
the flowchart above. The classification holds for the build whose hash was
checked in Lesson 10.2; a new build means reading the folder and the loaded
modules again.
