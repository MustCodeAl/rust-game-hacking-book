...
This is a **telemetry gap**, not a control bypass. It still matters because a
denial that leaves no event hides repeated mistakes and makes the control
difficult to test. The fix computes the decision first and then logs it,
...
Published evasion write-ups are usually operational recipes. Underneath the
commands, each one describes the same parts, and this lab's bypass contains
every one of them:

| Part | In this lab |
|---|---|
| The promise the control made | Read-only mode prevents state change |
| What the control actually checked | One command name |
| The equivalent effect it missed | Another command reached the same sink |
| The observable result | The weak policy returned true for `ApplyPatch` |
| Where the invariant belongs | At the shared state-changing boundary |
| The regression test that keeps it fixed | Every `ChangesState` variant is denied in read-only mode |

Described by its parts, a bypass is a gap between what a control promised and
what it checked. That is something a defender can close, and nothing in the
description is a step that could be aimed at someone else's machine.
...
flag is only a signal that often accompanies a violation; it is not the
invariant itself.

The four-outcome table above is called a **confusion matrix**. Testing a rule
against a controlled fixture fills it in one condition at a time: attached and
detached runs, slow and fast machines, and the diagnostic tools a legitimate
user is expected to run. A rule that fires on a legitimate debugger or profiler
lands in the false-positive row, and that is a compatibility bug worth fixing
rather than proof that every observer is hostile.
