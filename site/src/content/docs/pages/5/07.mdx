...
description: "Decompose recoil into input, weapon kick, spread, camera shake, and recovery; then separate each contribution by logging values over time."
chapter: "5.7"
minutes: 23
author: "attilathedud"
date: "2026-07-30"
...
In an offline AssaultCube session, log view angles while:
...
Log time as well as values. A single before-and-after pair shows total
movement; a short time series shows whether the kick happens instantly and how
recovery behaves across later frames.

Give each sample a monotonic timestamp and, when available, the frame or
simulation tick. Wall-clock timestamps can jump; a monotonic clock measures
elapsed duration. Each sample also needs the weapon, fire mode, and whether the
shot was accepted. Otherwise a reload or dry-fire sample can be mistaken for zero
recoil.
...
## Test one instruction with a temporary patch

A temporary debugger patch can show whether one instruction contributes to recoil. Keep a copy of the original bytes so you can restore them, change one instruction, and compare the same controlled shot.
