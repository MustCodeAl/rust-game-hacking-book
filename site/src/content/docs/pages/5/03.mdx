...
description: "Understand indexed drawing, observe OpenGL calls without stalling the render thread, and connect parameters to geometry and state."
chapter: "5.3"
minutes: 30
author: "attilathedud"
date: "2026-07-30"
sidebar:
  order: 3
  label: "5.3 Observe OpenGL Draw Calls"
---
...
```mermaid
sequenceDiagram
    participant R as Render thread, inside the hook
    participant C as Bounded channel
    participant W as Worker thread
    R->>R: copy mode, count, and type into a DrawSample
    R->>C: try_send the sample
    Note over R,C: If the channel is full, the sample is dropped and a counter goes up. The hook never waits.
    R->>R: call the real glDrawElements and return
    C-->>W: the worker takes samples whenever it runs
    W->>W: aggregate, label, and save
```
...
That warning is about **recursion**, a function calling itself. Recursion is a
normal technique when each call handles a smaller piece of the problem and one
case, the **base case**, answers without calling again. Finding your row in a
dark cinema works like that: you ask the person in front which row they are in,
they ask the person in front of them, and so on until someone in row 1 says
"row 1"; each answer then travels back with 1 added. Every question still
waiting for its answer is a **stack frame**, the return address and local values
that [Lesson 1.3](/pages/1/03/#the-stack-automatic-and-tied-to-function-calls)
showed each call keeping until it returns.

If the stored "original" pointer actually points at your wrapper, the wrapper
forwards every call to itself. It is asking itself which row it is in and waiting
for its own answer, so there is no base case. Each call adds another frame, the
thread's stack (1 MiB by default on Windows) fills after thousands of calls, and
Windows raises a stack-overflow exception that crashes the game, usually during
the very first draw call. That is the "Infinite recursion" row in the table at
...
Before and after the patch, the six bytes at `glDrawElements + 0x16` look like
this:

<MemoryStrip
  cells="+0x16=8B|+0x17=B6|+0x18=18|+0x19=0A|+0x1A=00|+0x1B=00"
  groups="0-1:mov into `esi` from `[esi + displacement]`|2-5:displacement `0xA18`, lowest byte first"
  caption="Before: one whole instruction. Read lowest byte first, `18 0A 00 00` is 0x18 + 0x0A × 256 = 24 + 2,560 = 2,584, which is `0xA18`."
/>

<MemoryStrip
  cells="+0x16=E9|+0x17=??|+0x18=??|+0x19=??|+0x1A=??|+0x1B=90"
  marks="0-4"
  groups="0-4:`jmp` to the cave: `E9` plus a 4-byte distance|5-5:`nop`"
  caption="After: the four `??` bytes hold the distance from `+0x1B`, the byte right after the jump (0x16 + 5 = 0x1B), to the cave, so they depend on where the DLL loaded. The cave returns to `+0x1C` (0x16 + 6 = 22 + 6 = 28 = 0x1C), so the `nop` never runs; it only fills the sixth byte so no fragment of the old instruction is left behind."
/>
