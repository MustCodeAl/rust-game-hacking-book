...
description: "Treat temporary color as a controlled classification experiment while accounting for materials, lighting, blending, and color space."
chapter: "5.4"
minutes: 20
author: "attilathedud"
date: "2026-07-30"
sidebar:
  order: 4
  label: "5.4 Color-Code Render Categories"
---
...
Purity also makes uncertainty visible. Return `Unknown` when the sample does not
match any category. A confident-looking wrong label is harder to diagnose than
an explicit unknown.
...
Keep the recorded samples separate from the classifier implementation. Then a new rule can be scored against the same samples, so a change in precision or recall comes from the rule and not from a different capture.
...
framebuffer, shader/program, and pass position to the draw sample before changing
the category rule.

## Identify a category with more than one number

A vertex count can be a quick first filter, not a durable identifier. Combine independent draw properties and return `Unknown` when they disagree.
