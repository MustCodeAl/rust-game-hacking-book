...
description: "Create a Lua 5.4 interpreter with mlua, load only needed libraries, expose typed host functions, and report script errors with context."
chapter: "12.3"
minutes: 35
author: "attilathedud"
date: "2026-08-14"
sidebar:
  order: 3
  label: "12.3 Embed Lua in the Host"
---
...
the same protocol Lesson 12.1 showed from the C side. The lab's
`game.distance(a, b)` takes two position tables and returns one number. Take
this call, and call its first table `p` and its second `q`:
...
```mermaid
flowchart LR
    V["Rust slice entities<br/>[0] local_player<br/>[1] bot_alpha<br/>[2] bot_bravo"] -->|"index + 1"| R["Lua table result<br/>[1] [2] [3]"]
    R -->|"key 1"| I["item table<br/>id = 0<br/>name = local_player<br/>alive = true"]
    I -->|"key position"| P["position table<br/>x = 12.0<br/>y = 8.0<br/>z = 2.0"]
```
