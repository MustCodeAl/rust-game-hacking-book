...
description: "Sample live game state without write rights, move file I/O off the observation path, count dropped events, and close a versioned log cleanly."
chapter: "4.8"
minutes: 35
author: "attilathedud"
date: "2026-08-06"
sidebar:
  order: 8
  label: "4.8 Build a Bounded Game Telemetry Recorder"
---
...
The same source is available as [`gold_telemetry.rs`](/windows-labs/src/bin/gold_telemetry.rs).
...
The tool should report roughly 300 accepted samples for 15 seconds (15 seconds × 20 samples per second = 300) and zero drops on a normal disk. A nonzero drop count is not hidden; it means the writer or chosen rate needs attention.
...
<Quiz
  ...
  answer="0"
  explanation="The debug loop should restore state and continue promptly. A bounded queue controls memory, nonblocking send avoids holding the target for disk I/O, and an explicit loss counter keeps lost events from going unnoticed."
/>
