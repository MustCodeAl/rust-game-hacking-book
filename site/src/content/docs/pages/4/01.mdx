...
description: "Reuse one verified player layout to read several local-match players and print a clean typed snapshot."
chapter: "4.1"
minutes: 18
author: "attilathedud"
date: "2026-07-30"
sidebar:
  order: 1
  label: "4.1 Build a Strategy-Game Observer"
---
...
therefore a copy of the values at one moment, not a live reference into Wesnoth.
...
The inline formula applies only once the debugger shows a consistent stride
...
Set a breakpoint on the gold update used at the end of a turn. Each time the instruction runs, read the object pointer it uses.
...
Two known players confirm it: their record addresses should differ by exactly one record size. That difference gives the stride, not yet the complete layout of a record.
...
The layout object keeps the offsets apart from the values read through them.
`gold: 4` means “gold sits four bytes from each record’s base in this build.”
Storing the number does not make it true; range checks and behavior tests
decide whether it still holds for this exact build.
...
A missing row means a bad layout, a changing process, or
an unreadable record, not that the player does not exist.
...
Those paths become a real reader:
