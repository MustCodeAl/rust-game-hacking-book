...
description: "Refactor separate prototypes into typed features with shared snapshots, clean toggles, and guaranteed restoration."
chapter: "8.8"
minutes: 25
author: "attilathedud"
date: "2026-07-30"
sidebar:
  order: 8
  label: "8.8 Turn Experiments into One Reliable Tool"
---
...
Reading other projects shows how several tools model the same problem. Test
their assumptions against your target, then write the smallest design that
fits. When code is actually reused, preserve its required
...
in-process menu should log the same fields for every state change:
`target_game`, `target_build`, `feature`, `address`, `expected_bytes`,
`found_bytes`, `action`, and `restored`. Those log entries show which patch was
enabled and whether its original bytes were restored. Keep them separate from
the visible menu so closing or redesigning the UI does not erase the log needed
to debug a failed cleanup.
