...
description: "Enumerate readable regions, scan copied bytes, filter candidates, and write only after an exact guarded check."
chapter: "7.5"
minutes: 35
author: "attilathedud"
date: "2026-07-30"
sidebar:
  order: 5
  label: "7.5 Build a Small Memory Scanner"
---
...
flowchart TD
    ...
    E --> F["Filter with the next scan"]
```

The Windows calls are only the outer shell. Candidate filtering uses checked local slices.

:::note
🔎 A filter pass re-reads only the addresses already on the candidate list, so it can remove candidates but never add them. If the list grows, the program rescanned memory instead of filtering; if it stops shrinking, check the value type, the range, and the filtering rule.
:::
...
How much faster is worth working out, because the same reasoning decides whether
any tool feature is usable. Suppose the game has 400 MiB of readable memory and
each candidate is a 4-byte value:

| Pass | What it reads | Bytes read |
|---|---|---|
| first scan | every readable byte: 400 × 1,048,576 | 419,430,400 |
| second scan | 40,000 candidates × 4 bytes | 160,000 |
| third scan | 18 candidates × 4 bytes | 72 |

The second pass reads about 2,600 times less than the first
(419,430,400 ÷ 160,000 ≈ 2,621), and it no longer depends on how big the game
is at all, only on how many candidates survived.

The first scan's cost grows in step with its input: give the game twice the
memory and the scan reads twice the bytes. Programmers write that as **O(n)**,
read "order n", where *n* is the size of the input. Some costs do not grow at
all: finding one candidate's previous value in a hash table keyed by address
takes about the same time with 40 candidates or 40,000, as
[Lesson 3.5](/pages/3/05/#a-hash-table-arithmetic-picks-the-bucket) explains.
That is **O(1)**, like shipping a box, which takes a day whether it holds one
game or ten. And a loop inside a loop multiplies. Comparing every one of the
40,000 candidates with every other takes 40,000 × 40,000 = 1,600,000,000
comparisons, 40,000 times the work of reading the list once, and doubling the
candidates makes it four times worse. That is **O(n²)**. Before adding a scan
feature, ask what happens to the work when the memory or the candidate list
doubles.
...
A misspelled string silently behaves like an unknown
...
This is a normal race, not necessarily a bug. Between the region query and the read, the game can free an allocation, change its protection, or exit. The scanner’s snapshot is always slightly behind the running process. A robust tool reports skipped work and continues with the candidates it can still read.
...
fn write_candidate(
    ...
    // the value the last scan saw before changing anything.
...
The full program is [`memory_scanner.rs`](/windows-labs/src/bin/memory_scanner.rs). It uses `VirtualQueryEx`, skips unreadable or oversized regions, reads at most 1 MiB per chunk, adds a three-byte overlap so a `u32` cannot hide across a chunk boundary, caps candidates at 250,000, handles disappearing regions, and prints at most 25 results.
...
| Candidate cap reached | Initial value is too common; start from a less common value |
...
| One address survives but write is refused | The value changed after filtering; scan again |

The refusal cases are successful safety behavior. They stop the program from writing to an address whose last read is out of date.
...
<Quiz
  ...
  answer="1"
  explanation="The first scan creates a candidate set. The second scan asks which members of that same set changed to the new value. Repeating controlled changes removes unrelated copies until a small, explainable group remains."
/>
