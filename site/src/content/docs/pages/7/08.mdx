...
description: "Combine scanning, instruction decoding, breakpoints, and bounded logging to observe real calls in the course games."
chapter: "7.8"
minutes: 35
author: "attilathedud"
date: "2026-07-30"
...
This scope is not just a performance setting. It gives the log a meaning. “These calls happened while one turn ended inside this verified module range” is a result you can reproduce. “The process called thousands of things” is mostly noise.
...
A bounded channel creates **backpressure**: it gives the producer a fixed amount of space instead of allowing logs to consume memory forever. `try_send` makes the policy explicit. Losing a counted event is better than holding every target thread suspended while a disk writer catches up.
...
- the logger moves events off the hot path;
