...
description: "Turn Windows debug events into a state machine, then break on Wesnoth’s real income instruction and inspect live gold updates."
chapter: "7.7"
minutes: 24
author: "attilathedud"
date: "2026-07-30"
sidebar:
  order: 7
  label: "7.7 Build a Minimal Windows Debugger"
---
...
On the course target, run the same input once without a debugger and once
attached, logging inputs, chosen branches, elapsed time, and output both times.
A difference that appears only while attached comes from the measurement, not
from the game's normal logic. That effect is called **instrumentation bias**:
the tool used to measure a program changes what the program does.

Debugger-aware behavior usually comes from a small set of sources: flags,
handles, exceptions, timing, instructions, memory, and interaction with the
desktop. A change that appears only while attached traces back to one of them.
This lesson intentionally does not turn those sources into concealment or bypass
instructions.
