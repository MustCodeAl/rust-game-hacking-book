...
Some production-server advice recommends compiling for the newest CPU your
machines guarantee. That is useful when you control the hardware. It is wrong
for these historical labs: `x86-64-v3` would produce a 64-bit program, while an
injected DLL here must be 32-bit. Treat build settings as part of the program's
contract—choose the strongest target that **every intended machine and target
process actually supports**, not the strongest target your own PC happens to
support.
