...
|---|---|---|
| Coverage gap | Which execution, input, or memory paths does the control not observe? | Send labeled events through your own toy paths and compare logs |
...
only when the target build matches, the address belongs to a permitted region,
...
```rust
fn authorize_lab_write(request: &WriteRequest, state: &LabState) -> Result<(), Denial> {
    // ✅ Each independent assumption gets a named, testable rejection.
    state.authorization.allows_writes().then_some(()).ok_or(Denial::ReadOnly)?;
