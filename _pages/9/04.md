...
The tool rejects tabs and newlines in paths because those characters would make the small text format ambiguous. Production formats often use JSON, CBOR, or signed catalogs; this deliberately tiny format keeps the parsing lesson visible.
...
Streaming keeps memory use roughly constant. A 4 GB game archive does not require a 4 GB Rust vector just to calculate its fingerprint.
...
For a classroom lab, close the game and mod tools while verifying. A production launcher would need a stronger design, such as keeping an opened file handle, using signed packages, or letting a trusted updater own both verification and launch.
