...
Think of a virtual address as an apartment number and a physical address as the actual storage location. Two processes may both use virtual address `0x1000`, yet their page tables send those addresses to different physical pages.
...
If a protected machine blocks unapproved DMA, that is the system working correctly. Do not turn protections off just to complete a lesson. Use the synthetic capture created by the lab tests or an offline capture produced by an authorized hypervisor workflow.
...
The useful computer-science problem is translating and validating memory obtained through an authorized capture workflow. The complete Rust lab:
