...
When the crosshair is over a player, `eax`/`edi` is non-zero. Looking at empty space produces zero. Replace the five-byte call at `0x0040AD9D` with a verified detour, execute the original call inside the cave, then publish the result to Rust:
...
Keep this as an offline bot-match exercise. The hook and firing behavior above are intentionally complete for the authorized open-source target; do not take it into a public match.
