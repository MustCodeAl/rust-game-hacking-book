...
Virtual addresses belong to one process. The same numeric address in two different processes can name completely different physical memory, and a freed address can later be reused for another allocation in the same process. Treat an address as a temporary coordinate, not an object identity.
