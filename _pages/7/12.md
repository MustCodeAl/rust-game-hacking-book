...
chapter: "7.12"
minutes: 28
summary: Copy authorized game code once, divide immutable local snapshots across workers, cap results, cancel broad scans, and restore deterministic output.
---
...
Only the capture phase touches the running Wesnoth process. Workers scan ordinary `Vec<u8>` buffers owned by the Rust tool.
...
- the matching algorithm is safe Rust;
...
The real tool also caps all copied executable bytes at 128 MiB. Rust prevents memory corruption, but it does not prevent a logically unbounded allocation from exhausting the machine.
...
{% include quiz.html
  ...
  answer="0"
  explanation="Remote memory changes and Windows reads can fail. Copying once keeps that unstable boundary in one stage; workers then perform deterministic safe-Rust comparisons over immutable slices."
%}
