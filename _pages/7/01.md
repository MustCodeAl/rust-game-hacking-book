---
title: Inject Your Rust DLL into an Authorized Game
author: attilathedud
date: 2026-07-30
...
chapter: "7.1"
minutes: 20
summary: Build a Rust DLL injector for the exact open-source game versions used in this book and control every Windows resource lifetime.
mermaid: true
---

## Scope this tool to the course games

This injector is for the exact open-source, offline targets used in the book. Put the allowlist in code so a mistyped command cannot inject into an unrelated process.
...
Run the games in an offline match or against the local server you control. This chapter does not cover anti-cheat evasion, protected processes, or injecting into someone else’s software.

## The classic loader sequence

```mermaid
flowchart TD
    A["Open authorized game process"] --> B["Allocate remote memory"]
...
For the versions in this book, install the 32-bit Rust target and build the DLL for it:
...
The completed tool performs a real injection into the authorized course games while still behaving like good Rust: exact target checks, owned handles, bounded waits, clear errors, and automatic cleanup.
