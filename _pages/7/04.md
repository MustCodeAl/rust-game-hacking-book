...
It does not recover the original Rust or C++ source. Compiler variable names,
...
fn disassemble(bytes: &[u8], bitness: u32, start_ip: u64) -> anyhow::Result<Vec<String>> {
    ...
    // not say where this local byte slice is stored in the Rust process.
...
Rust tool. A 64-bit Windows system can run the 32-bit Wesnoth course build.
