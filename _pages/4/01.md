...
therefore a record of one observation, not a live Rust reference into Wesnoth.
...
Create a two-local-player match, end one turn for each player, and print indices `0` and `1`. Spend gold with each side separately. Only the matching row should change. You have now rebuilt the original Wesnoth stathack in Rust.
