...
chapter: "10.8"
minutes: 34
summary: Poll a hotkey, send balanced input, compare window messages, and choose the smallest Windows API for an authorized local game tool.
---
...
| `SendInput` | Inserts keyboard or mouse events into the system input stream | Drive an authorized offline lab while its window is active | It does not target one `HWND` |
...
fn key_is_down(key: VIRTUAL_KEY) -> bool {
    ...
    // only samples desktop input state and does not retain any Rust pointer.
...
check that the authorized target owns the foreground window before sending:
...
you own both sides and define a pointer-free contract.
...
| Is the authorized target active? | `GetForegroundWindow` | Pause synthetic input when it is not |
