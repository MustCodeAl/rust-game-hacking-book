...
layout: post
permalink: /pages/4/08/
chapter: "4.8"
minutes: 35
summary: Sample live game state without write rights, move file I/O off the observation path, count dropped events, and close a versioned log cleanly.
mermaid: true
---
...
promise to the learner—for example, inventing continuous data after samples
were lost or labeling an incomplete file as clean.

The design cannot prevent every fault. It can keep each one from becoming a
misleading result:

| Fault during a run | Honest recorder response |
|---|---|
| queue reaches 128 events | apply the declared drop policy and increment `dropped` |
| writer disconnects | stop recording and return an error |
| target memory becomes unreadable | mark the run incomplete; never repeat the last value as if it were new |
| an older reader sees a new schema | reject the unknown version or use a reader written for it |
| shutdown begins | close producers, drain accepted events, flush, and join the writer |

This is why counters, versions, and a clean-shutdown marker are part of the data
model. They let a later graph distinguish “gold stayed at 100” from “the tool
stopped observing for two seconds.” 📉
...
Backpressure is not one library call. It is the decision a system makes when producers outrun consumers.
...
That promise is deliberately narrow. `BufWriter::flush` sends buffered bytes to
the file object, but it is not a guarantee that every storage device could
survive an immediate power loss. A short course telemetry run needs honest
completion and visible loss; a crash-resilient journal would need a stronger,
explicit durability design.
